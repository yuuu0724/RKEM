...
int main(int argc, char* argv[]) {
    ...
    fprintf(stdout, "[INFO] OCR process starting...\n");
    fflush(stdout);
    ...
    LOG_INFO("OCR process starting...");

    // 解析参数
    std::string camera_device = "/dev/video21";
    std::string det_model = "model/ocr/PP-OCRv5_mobile_det.rknn";
    std::string rec_model = "model/ocr/PP-OCRv5_mobile_rec.rknn";
    std::string dict_file = "model/ocr/dataset.txt";
    std::string ipc_socket = "/tmp/integrated_inspection_ocr.sock";
    ...
    fprintf(stdout, "[INFO] OCR: camera=%s, ipc=%s\n", camera_device.c_str(), ipc_socket.c_str());
    fflush(stdout);

    // 初始化 OCR 引擎
    fprintf(stdout, "[INFO] Initializing OCR engine...\n");
    fflush(stdout);
    OcrEngine ocr_engine;
    if (!ocr_engine.Init(det_model, rec_model, dict_file)) {
        fprintf(stderr, "[ERROR] Failed to init OCR engine\n");
        return 1;
    }
    ...
    LOG_INFO("OCR process initialized successfully");
    ...
    while (g_running) {
        ...
        if (!camera.GetFrame(frame, 1000)) {
            if (!camera.IsCapturing()) {
                LOG_ERROR("Camera stopped, exiting OCR process");
                break;
            }
            continue;
        }

        if (!ShowOpenCvFrame("OCR Camera", frame, show_window)) {
            delete[] frame.data;
            g_running = false;
            break;
        }
        ...
        // 提交 OCR 任务到线程池
        try {
            thread_pool.Submit([&ocr_engine, &ipc, frame]() {
                std::vector<OcrResult> results;

                // 执行 OCR 识别
                if (ocr_engine.Recognize(frame.data, frame.width, frame.height,
                                         frame.channels, results)) {
                    // 发送结果
                    for (const auto& result : results) {
                        ipc.SendOcrResult(result);
                        Logger::GetInstance().LogOcrResult(result);
                    }
                }
            ...
            LOG_ERROR("Failed to submit OCR task: %s", e.what());
            delete[] frame.data;
            break;
        }
    }
    ...
    // 清理
    LOG_INFO("OCR process shutting down...");
    thread_pool.Stop();
    camera.StopCapture();
    ipc.Shutdown();
    ocr_engine.Release();
    if (show_window) {
        cv::destroyWindow("OCR Camera");
    }
